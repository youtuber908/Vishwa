...
console.log('Loaded  regions from indiaRegions.ts');
...
for (const [normName, features] of Object.entries(nameToFeatures)) {
  ...
  if (!expectedIdToFeatures[expectedId]) {
    console.warn('Expected ID  not found in INDIA_REGIONS for GeoJSON name ');
...
for (const [expectedId, features] of Object.entries(expectedIdToFeatures)) {
  if (features.length === 0) {
    console.warn('No features found for expected ID: ');
    continue;
  }
  ...
  try {
    ...
    console.error('Failed to generate path for  (ID: ):', e.message);
  }
}
