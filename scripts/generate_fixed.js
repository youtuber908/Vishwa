// Auto-generated from india-states.geojson (Natural Earth data)
// We'll keep same viewBox comment as before
// Projection: equirectangular (simple), viewBox: 341 335 332 328
...
// From inspection, it's "name" (not NAME_1)
const nameProp = "name";
...
// Manual mapping for known mismatches (GeoJSON name -> expected id)
const manualMapping = {
  // From previous runs we saw:
  "Orissa": "odisha",
  "Uttaranchal": "uttarakhand",
  "Andaman and Nicobar": "andaman-nicobar",
  // Add the two missing ones
...
// Note: each feature is already a single region, but we keep the structure for potential merging
...
for (const [normName, features] of Object.entries(nameToFeatures)) {
  ...
  if (!expectedId) {
    console.warn(`Could not determine expectedId for GeoJSON name ${geoName}`);
    continue;
  }
...
for (const [expectedId, features] of Object.entries(expectedIdToFeatures)) {
  ...
  try {
    ...
    results[expectedId] = { id: expectedId, name: name, d: pathData };
  } catch (e) {
...
// Add the INDIA_MAP_VIEWBOX export
